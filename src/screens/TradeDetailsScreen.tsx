...
import { formatFullCurrency, formatDate, formatDateTime } from '../utils/formatters';
import { RootStackParamList } from '../types/trade';

type DetailsRoute = RouteProp<RootStackParamList, 'TradeDetails'>;
...
export default function TradeDetailsScreen() {
  ...
  const trade = useMemo(
    () => MOCK_TRADES.find((t) => t.id === tradeId),
    [tradeId]
  );
  ...
  const detailRows = [
    ...
    { label: 'Transaction', value: `${isPurchase ? 'Purchase ↑' : 'Sale ↓'} • Code ${trade.transactionCode.toUpperCase()}` },
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.headerSection}>
        ...
        <Text style={styles.ticker}>{trade.ticker} • {trade.sector}</Text>
      ...
      <View style={styles.signalCard}>
        ...
        <Text style={styles.signalValue}>
          {formatFullCurrency(trade.value)} fictional demo insider {trade.type}
        </Text>
      </View>
